...
const LIST_DIVISI = ["Semua", "Inti Kabinet", "Human Resources Development", "Internal Relations", "External Relations", "Democratic Advocacy", "Research and Education", "Business and Enterpreneur", "Creative Media"];

export default function KabinetPage() {
  const [divisiAktif, setDivisiAktif] = useState("Semua");

  const anggotaTersaring = divisiAktif === "Semua" ? DATA_KABINET : DATA_KABINET.filter((anggota) => anggota.category === divisiAktif);

  return (
    <main className="bg-canvas min-h-screen py-24">
      <div className="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-10">
        {/* LAYOUT HEADER: Judul Kiri, Dropdown Kanan (Pada desktop) */}
        <div className="flex flex-col md:flex-row md:items-end md:justify-between border-b border-hairline pb-8 mb-12 gap-6">
          <div>
            ...
            <div className="h-0.5 w-16 bg-linear-to-r from-m-blue-dark to-m-red mt-3" />
          </div>

          {/* COMPONENT DROPDOWN FILTER (Aman & Responsif) */}
          <div className="flex flex-col gap-1.5 w-full md:w-72">
            ...
            <div className="relative w-full">
              <select
                ...
                className="w-full appearance-none rounded-none border border-hairline bg-surface-soft px-4 py-3 pr-10 text-xs font-bold uppercase tracking-wider text-on-dark transition duration-200 outline-none cursor-pointer focus-visible:border-m-blue-dark focus-visible:ring-1 focus-visible:ring-m-m-blue-dark"
              >
                {LIST_DIVISI.map((divisi) => (
                  <option key={divisi} value={divisi} className="bg-surface-soft text-on-dark font-sans normal-case text-sm">
                    {divisi === "Semua" ? "✨ Tampilkan Semua Divisi" : divisi}
                  </option>
                ))}
              </select>
        ...
        {/* GRID FOTO ANGGOTA KABINET */}
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6 sm:gap-8">
          {anggotaTersaring.map((anggota) => (
            <div key={anggota.id} className="group relative flex flex-col border border-hairline bg-surface-soft p-4 shadow-xs transition-all duration-300 hover:shadow-md">
              <div className="relative aspect-[3/4] w-full overflow-hidden bg-black border border-hairline/40">
                <img src={anggota.image} alt={anggota.name} className="h-full w-full object-cover transition-transform duration-500 ease-out group-hover:scale-105" />
              </div>
