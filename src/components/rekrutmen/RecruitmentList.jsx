...
export default function RecruitmentList({ kategori = "hmbd" }) {
  ...
  // Mengubah list opsi tag agar lebih relevan
  ...
  const renderCard = (job) => {
    // =========================================================================
    // LOGIKA DINAMIS OTOMATIS BERDASARKAN REAL-TIME HARI INI (1 JULI 2026)
    // =========================================================================
    const today = new Date();
    today.setHours(0, 0, 0, 0); // Reset jam agar murni kalkulasi tanggal

    // Set batas waktu penutupan tepat pada jam 23:59:59 di hari tersebut
    const closingDateTime = new Date(`${job.closingDate}T23:59:59`);

    // Status ditentukan secara otomatis berdasarkan perbandingan waktu
    const isClosed = today > closingDateTime;
    const isOpen = !isClosed;

    // Menghitung sisa hari (Urgensi untuk pendaftar)
    const diffTime = closingDateTime - today;
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));

    // Teks label dinamis untuk status rekrutmen
    ...
    return (
      <li key={job.id} className="flex flex-col border border-hairline bg-surface-soft p-5 transition duration-300 ease-out hover:border-m-blue-dark/40 hover:shadow-[0_12px_40px_rgba(28,105,212,0.12)]">
        {/* BINGKAI FOTO / POSTER */}
        <div className={`relative w-full overflow-hidden border border-hairline bg-canvas p-2 ${isEksternal ? "aspect-square" : "aspect-[16/10]"}`}>
          <div className="relative h-full w-full overflow-hidden border border-hairline group">
            ...
            {!hasImageError && job.image && (
              <Image src={job.image} alt={job.title} fill sizes="(max-w: 768px) 100vw, 50vw" className="object-cover opacity-90 transition duration-500 ease-out group-hover:scale-105" onError={() => handleImageError(job.id)} />
            )}
        ...
        {/* FOOTER KARTU: SELESAI / AKSES TOMBOL FORM */}
  ...
  return (
    <div className="space-y-12">
      {/* COMPONENT DROPDOWN FILTER (Sesuai Gaya Desain Halaman Kabinet) */}
      {kategori === "lainnya" && (
        <div className="flex flex-col md:flex-row md:items-end md:justify-between border-b border-hairline pb-8 mb-4 gap-6">
          <div>
            ...
            <div className="h-0.5 w-16 bg-linear-to-r from-m-blue-dark to-m-blue-light mt-3" />
          </div>
          ...
          <div className="flex flex-col gap-1.5 w-full md:w-72">
            ...
            <div className="relative w-full">
              ...
              {/* Pengunci Panah Kustom Kanan — Anti Melar di Layar Mobile */}
        ...
        <div className="space-y-16">
          {BDV_ITEMS.length > 0 && (
            <div className="border-t-2 border-m-blue-dark pt-6">
              ...
              <ul className="grid gap-8 md:grid-cols-2">{BDV_ITEMS.map(renderCard)}</ul>
            </div>
          )}
          {DIGIFEST_ITEMS.length > 0 && (
            <div className="border-t-2 border-hairline pt-6">
              ...
              <ul className="grid gap-8 md:grid-cols-2">{DIGIFEST_ITEMS.map(renderCard)}</ul>
            </div>
          )}
        </div>
      )}
      ...
      {kategori === "lainnya" && (
        <div>
          <ul className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">{filteredByTag.map(renderCard)}</ul>
        </div>
      )}

      {/* Notifikasi data kosong */}
      {filteredByTag.length === 0 && (
        <div className="border border-hairline bg-surface-soft p-12 text-center">
